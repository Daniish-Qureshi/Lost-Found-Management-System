...
export async function addItem(payload) {
  ...
  // Determine ownerId from Firebase Auth if available (preferred) so rules
  // requiring request.auth.uid == ownerId will pass. Fall back to payload.ownerId.
  let ownerId = payload?.ownerId
  try {
    const auth = getAuth()
    if (auth && auth.currentUser && auth.currentUser.uid) ownerId = auth.currentUser.uid
  } catch (e) {
    // ignore if auth isn't configured
  }
  ...
  const raw = {
    ...
    // default status to 'active' unless caller set otherwise
...
// Debug helper (dev-only): attempt a minimal write to Firestore and return result or throw error.
...
// Expose helper in development for quick browser console tests
if (typeof window !== "undefined" && process.env.NODE_ENV !== "production") {
  // eslint-disable-next-line no-console
  console.log('[firestore-client] attaching debugWrite to window.debugWrite (dev only)')
  // attach as an async function returning a promise
